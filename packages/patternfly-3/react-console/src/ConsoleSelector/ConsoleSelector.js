...
import { Grid, Row, Col, Form, FormGroup, Dropdown, MenuItem } from 'patternfly-react';
...
class ConsoleSelector extends React.Component {
  ...
  onChangeDisconnectBySwitchClick(target) {
    this.setState(prevState => {
      const actualType = prevState.keptConnection[prevState.type];
      return {
        disconnectByChange: target.checked,
        keptConnection: target.checked ? { [actualType]: true } : prevState.keptConnection
      };
    });
  }
